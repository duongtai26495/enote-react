...
const WorkspaceCard = ({ wsItem, removeWs, type }) => {
    ...
    return (
        type === COLS
            ?
            <div className={`rounded mb-1 w-full flex py-4 px-2 relative border-b  border-b-slate-300 bg-white overflow-hidden justify-between items-center lg:hover:shadow-lg transition-all`}>
                ...
                <div className='py-2 w-1/2 lg:w-1/3 overflow-x-hidden'>
                    {
                        isUpdateName ?
                            <input
                                autoFocus
                                className={`bg-transparent outline-none border px-1 rounded-md text-slate-600 w-full `}
                                type='text'
                            ...
                            />
                            :
                            <Link to={`/workspace/${workspaceItem.id}`} className='w-full flex flex-col justify-start font-bold'>
                                {workspaceItem.name}
                            </Link>
                    }

                    <span style={{ fontSize: "10px" }} className='text-start flex items-center gap-1'>
                    <svg className='w-3 h-3' viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><defs></defs><title/><g dataname="Layer 15" id="Layer_15"><path class="cls-1" d="M16,31A15,15,0,1,1,31,16,15,15,0,0,1,16,31ZM16,3A13,13,0,1,0,29,16,13,13,0,0,0,16,3Z"/><path className="cls-1" d="M20.24,21.66l-4.95-4.95A1,1,0,0,1,15,16V8h2v7.59l4.66,4.65Z"/></g></svg>
                    {getTheTime(workspaceItem.updated_at)}
                    </span>
                </div>

                <div className='flex w-1/2 lg:w-2/3 flex-col-reverse lg:flex-row item-center justify-center my-1 gap-2'>

                    <Link to={`/workspace/${workspaceItem.id}`} className='flex flex-col justify-center items-end lg:items-center w-full'>
                        <span className='text-xs lg:block hidden text-center font-bold'>{workspaceItem.created_at}</span>
                        <span className='text-xs text-center flex items-center gap-1 font-bold'>
                        ...
                        {getTheTime(workspaceItem.created_at)})
                        </span>
                    </Link>
                    <div className='flex w-full justify-end items-center gap-2'>

                        <span className={`w-8 h-8 justify-center text-xs text-white flex items-center rounded-full bg-slate-500 ${workspaceItem.note_count > 0 ? "p-2" : "hidden"}`}>{workspaceItem.note_count > 0 ? "+" + workspaceItem.note_count : ""}</span>
                        {
                            isUpdateName ?
                        ...
                        <button onClick={toggleFavorite} className={`${workspaceItem.favorite ? "bg-amber-200" : "bg-slate-400"} w-8 h-8 rounded-sm transition-all`}>
                            <svg className={`mx-auto w-5 h-5 ${workspaceItem.favorite ? "fill-amber-500" : "fill-slate-600"}`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M19 10.132v-6c0-1.103-.897-2-2-2H7c-1.103 0-2 .897-2 2V22l7-4.666L19 22V10.132z" /></svg>
                        </button>
                    </div>
            ...
            <div className={`rounded mb-5 w-full flex-col relative bg-white  border shadow-sm overflow-hidden justify-between items-center lg:hover:shadow-lg transition-all`}>
...

                {
                    isUpdateName ?
                        ...
                        :
                        <Link to={`/workspace/${workspaceItem.id}`} className={`py-5 w-full text-center overflow-x-hidden px-2`}>
                ...
                <div className='mx-auto  border-opacity-50  w-full py-3 flex item-center justify-center gap-2'>
