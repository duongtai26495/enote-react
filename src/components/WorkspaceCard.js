    ...
    return (
        type === COLS
            ...
            <div className={`rounded mb-1 w-full flex py-4 px-2 relative border-b  border-b-slate-300 bg-white bg-opacity-50 overflow-hidden justify-between items-center lg:hover:bg-slate-300 transition-all`}>
                ...
                <div className='py-2 w-1/2 lg:w-1/3 overflow-x-hidden'>
                    {
                        isUpdateName ?
                            ...
                            <Link to={`/workspace/${workspaceItem.id}`} className='w-full flex flex-col justify-start '>
                                <h3 className='font-bold'>{workspaceItem.name}</h3>
                            </Link>
                    }
                ...
                <div className='flex w-1/2 lg:w-2/3 flex-col-reverse lg:flex-row item-center justify-center my-1 gap-2'>
                    ...
                    <div className='flex w-full justify-end items-center gap-2'>
                        ...
                        <span className={`w-10 h-10 text-xs text-white flex items-center rounded-full bg-slate-500 ${workspaceItem.note_count > 0 ? "p-2" : "hidden"}`}>{workspaceItem.note_count > 0 ? "+" + workspaceItem.note_count : ""}</span>
                        {
                            isUpdateName ?
            ...
            <div className={`rounded mb-5 w-full flex-col relative bg-transparent border shadow-sm overflow-hidden justify-between items-center lg:hover:shadow-lg transition-all`}>
                ...
                <span onClick={() => navigate(`/workspace/${workspaceItem.id}`)} style={{ fontSize: "10px" }} className='cursor-pointer text-center block'>({getTheTime(workspaceItem.updated_at)})</span>
                        </Link>
                }
