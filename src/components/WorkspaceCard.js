...
const WorkspaceCard = ({ wsItem, removeWs }) => {
    ...
    return (
        <div className={`border-l-4 border-l-blue-500 w-full flex px-2 mb-2 relative bg-white overflow-hidden justify-between items-center rounded-md lg:hover:shadow-lg transition-all`}>
            ...
            <div className='lg:w-1/3 py-2'>
                {
                    isUpdateName ?
                        ...
                        />
                        :
                        <Link to={`/workspace/${wsItem.id}`} className='w-fit flex flex-col justify-start '>
                            <h3 className='whitespace-nowrap font-bold'>{wsItem.name}</h3>
                        </Link>
                }
                
                <span style={{ fontSize: "10px" }} className='text-start block'>({getTheTime(wsItem.updated_at)})</span>
            </div>
            <div className='flex w-2/3 flex-col-reverse lg:flex-row item-center justify-center my-1 gap-2'>
                <p className='flex justify-center items-center w-full'>
                    <span className='text-xs text-center block font-bold'>{wsItem.created_at}</span>
                </p>
                <div className='flex w-full justify-end items-center gap-2'>
                    {
                        isUpdateName ?
                            <button className='w-7 h-7 p-1 scale-90 rounded-sm bg-green-100' onClick={() => updateNewName()}>
                            ...
                            :
                            <button className='w-7 h-7 p-1 scale-90 rounded-sm bg-blue-100' onClick={() => setUpdateName(true)}>
                    ...
                    <button className='w-7 h-7 p-2 rounded-sm bg-red-100' onClick={toggleConfirm}>
