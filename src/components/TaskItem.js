...
const TaskItem = ({ task, noteId, updatePercentage, isUpdateList, deleteTaskId, isExpandTask }) => {
    ...
    const RenderTask = memo(() => {
        return (
            <div className={`${isExpandTask ? "h-fit" : "h-0"} overflow-y-hidden transition-all bg-transparent m-2 rounded-md flex-1 flex flex-row gap-3 justify-between items-start px-2 relative border`}>
