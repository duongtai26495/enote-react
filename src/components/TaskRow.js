...
const TaskRow = ({ task, noteId, updatePercentage, isUpdateList, deleteTaskId }) => {
    ...
    return (
        <div className={`h-fit ${taskItem.type === "NOTE" ? "bg-orange-50" : "bg-white"}  lg:hover:shadow-lg overflow-y-hidden transition-all bg-transparent my-1 flex-1 flex flex-row gap-5 justify-between items-center px-2 relative border`}>
            ...
            <div className={`${isUpdateList && deleteTaskId === taskItem.id ? "visible" : "invisible"} absolute top-0 left-0 w-full h-full bg-white rounded-md flex flex-row items-center justify-center`}>
                <img src={loading_gif} className='w-8 h-8' />
                    ...
                    />
            <div className='flex flex-col items-center gap-2 w-fit border-l pl-3 pb-1 pt-2'>
                <div className='flex flex-row w-14 items-center gap-3'>

                    <div onClick={toggleNewState} className={`${taskItem.type === "CHECK" ? "visible" : "invisible"} cursor-pointer h-4 w-4 text-start border relative`}>
                    ...
                    <p onClick={toggleSubSetting} className='cursor-pointer lg:hover:scale-125 transition-all w-fit'>
