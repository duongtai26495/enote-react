...
const TaskRow = ({ task, noteId, updatePercentage, isUpdateList, deleteTaskId }) => {
    ...
    return (
        <div className={`h-fit bg-white overflow-y-hidden transition-all bg-transparent my-2 mr-0 lg:mr-2 rounded-md flex-1 flex flex-row gap-5 justify-between items-center px-2 relative border`}>
            ...
            <div className='flex flex-col items-center gap-2 w-fit border-r pr-3 pb-1 pt-2'>
                ...
                <span className={`h-5 w-fit text-center text-xs text-slate-400 italic whitespace-nowrap`}>{task.updated_at && getTheTime(task.updated_at)}</span>
            </div>
