...
const Breadcrumbs = ({className, text, localtion = -1}) => {
    const navigate = useNavigate()
    return (
        <div className={`bg-slate-300 p-2 ${className}`}>
            
            <button className='p-2 flex flex-rpw gap-2 lg:hover:bg-slate-100 text-xs rounded bg-white shadow whitespace-nowrap' onClick={() => navigate(localtion)}>
